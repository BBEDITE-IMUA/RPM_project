...
async def get_personal_account_message():
    # "👤 <b>Личный кабинет</b>\n\n"
    #     "Здесь ты мож
    return _('📜<b> Choose an action</b>')


async def change_language_message():
    return _('🌍 <b>Choose language</b>')
...
async def get_my_subscription_message(user_id: int) -> str:
    ...
    if not keys:
        return _("👤 <b>Personal account</b>\n\n' 'You don't have any active subscriptions 📦\n")

    keys_text = '\n'.join(
        _('Key: <code>{key}</code>, Status: <b>{status}</b>, Expiry Date: {expiry_date}').format(
            key=key['value'],
            status=key['status'],
