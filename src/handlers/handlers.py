...
async def agreement_message(call: CallbackQuery, state: FSMContext) -> None:
    await call.message.edit_text(
        await texts.get_agreement_message(), reply_markup=await keyboards.get_agreement_keyboard()
    )
...
async def agree_with_terms(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await call.message.edit_text(
        await texts.get_agree_with_terms_message(), reply_markup=await keyboards.get_agree_with_terms_keyboard()
    )
...
async def disagree_with_terms(call: CallbackQuery, state: FSMContext) -> None:
    await call.message.edit_text(
        await texts.get_disagree_with_terms_message(), reply_markup=await keyboards.get_disagree_with_terms_keyboard()
    )
...
async def start_again(call: CallbackQuery, state: FSMContext) -> None:
    await call.message.edit_text(
        await texts.get_start_again_message(user=call.from_user.full_name),
        reply_markup=await keyboards.get_start_keyboard(),
    )
...
async def personal_account(call: CallbackQuery, state: FSMContext) -> None:
    await call.message.edit_text(
        await texts.get_personal_account_message(), reply_markup=await keyboards.get_personal_account_keyboard()
    )
...
async def my_subscription(call: CallbackQuery, state: FSMContext) -> None:
    await call.message.edit_text(
        await texts.get_my_subscription_message(), reply_markup=await keyboards.get_personal_account_keyboard()
    )
...
async def change_language(call: CallbackQuery, state: FSMContext) -> None:
    await call.message.edit_text(
        await texts.change_language_message(), reply_markup=await keyboards.change_language_keyboard()
    )
